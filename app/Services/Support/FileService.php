...
class FileService
{
    ...
    public function getBase64imgInfo($base64img, $type)
    {
    ...
    public function uploadBase64($base64img, $dir = 'images')
    {
        if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64img, $result)) {

            $ext = $result[2];
            $name = $dir . '/' . uniqid() . '.' . $ext;
            if (Storage::disk('public')->put($name, base64_decode(str_replace($result[1], '', $base64img)))) {
                return $name;
            }
