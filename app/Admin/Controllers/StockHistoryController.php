...
class StockHistoryController extends AdminController
{
    ...
    protected function grid()
    {
        return Grid::make(new StockHistory(['sku.product']), function (Grid $grid) {
            ...
            $grid->combine('入库信息', ['in_num',  'in_price']);
            $grid->combine('出库信息', ['out_num',  'out_price']);
            $grid->combine('产品信息', ['sku.product.item_no', 'sku.product.name', 'sku.product.unit.name', 'sku.product.type_str', 'sku.attr_value_ids_str']);
